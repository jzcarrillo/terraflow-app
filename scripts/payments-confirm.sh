...
echo -e "\033[0;36m=== Confirm Payment Test ===\033[0m"
...
echo -e "\n\033[0;33mConfirming payment ID: $PAYMENT_ID\033[0m"
RESPONSE=$(curl -s -X PUT "$API_GATEWAY_URL/api/payments/$PAYMENT_ID/confirm" \
    -H "Authorization: Bearer $TOKEN")

if [ $? -eq 0 ]; then
    echo -e "\033[0;32mSuccess: Payment confirmed\033[0m"
    echo "$RESPONSE" | jq -r '"Message: \(.message)\nTransaction ID: \(.transaction_id)\nNew Status: PAID"'
    echo -e "\033[0;33mNote: This will trigger land title activation\033[0m"
else
    echo -e "\033[0;31mFailed\033[0m"
fi

echo -e "\n\033[0;36m=== Confirm Payment Completed ===\033[0m"
