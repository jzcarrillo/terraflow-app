Feature: Mortgage Payment
  ...
  # Payment Flow:
  # 1. Create mortgage → PENDING status
  # 2. Create payment with reference_type: 'mortgage'
  # 3. Confirm payment → triggers PAYMENT_STATUS_UPDATE event
  # 4. handleMortgagePayment() → PENDING to ACTIVE
  # 5. Record to blockchain → store blockchain_hash
  ...
  Scenario: Successfully pay mortgage fee to activate
    ...
    When I create a payment with:
      ...
      | reference_id   | 1        |
      | amount         | 5000     |
    And I confirm the mortgage payment
    Then the mortgage payment status should be "PAID"
  ...
  Scenario: Cannot activate mortgage if 3 ACTIVE mortgages already exist
    ...
    And I should see mortgage error "Cannot activate mortgage: Maximum 3 active mortgages already reached"
  ...
  Scenario: Edit mortgage payment details while PENDING
    ...
    Then the mortgage payment should be updated successfully
    And the payment amount should be 6000

  Scenario: Cancel PENDING mortgage payment
    Given mortgage payment id 1 has status "PENDING"
    When I cancel the mortgage payment
    Then the mortgage payment status should be "CANCELLED"
  ...
  Scenario: Cannot cancel PAID mortgage payment
    ...
    And I should see mortgage error "Cannot cancel PAID mortgage payment"
