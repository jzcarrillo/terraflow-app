...
async function automateLandRegistration() {
  ...
  try {
    ...
    await page.goto('http://localhost:30081/register', { waitUntil: 'networkidle' });
    ...
    await page.goto('http://localhost:30081/login', { waitUntil: 'networkidle' });
    await page.waitForTimeout(2000);
    ...
    if (!paymentTabFound) {
      ...
      await page.goto('http://localhost:30081/payments', { waitUntil: 'networkidle' });
    }
    ...
    if (errorMessage > 0) console.log('  ✅ Error displayed: Maximum of 3 mortgages reached');
    await page.waitForTimeout(1000);
    const closeSelectors = ['button:has-text("Close")', 'button:has-text("Cancel")', '[aria-label="Close"]'];
    for (const selector of closeSelectors) {
      if (await page.locator(selector).count() > 0) { await page.click(selector); break; }
    }
    await page.waitForTimeout(1000);
    ...
    for (const selector of dialogCloseSelectors2) {
      if (await page.locator(selector).count() > 0) { await page.click(selector); await page.waitForTimeout(500); break; }
    }
    await page.waitForTimeout(1500);
    await page.getByRole('button', { name: 'Transfer Title' }).click();
    await page.waitForTimeout(2000);
